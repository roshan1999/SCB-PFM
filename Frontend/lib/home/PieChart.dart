...
List<Category> kCategories = [
  ...
  Category('eating', amount: 190.00),
  Category('bills', amount: 160.00),
  Category('subscriptions', amount: 100.00),
  Category('fees', amount: 120.00),
];
...
class Transaction {
  final String label;
  Transaction({@required this.label, @required this.amount, @required this.date ,@required this.description});
...
List <Transaction> getTransaction(String label){
  List <Transaction> result;
  for(int i =0;i<kTransactions.length;i++){
    if(kTransactions[i].label == label){
      result.add(kTransactions[i]);
    }
  }
  return result;
}