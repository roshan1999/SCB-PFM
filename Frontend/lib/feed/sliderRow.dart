...
class SildeAbleRow extends StatelessWidget {
  ...
  Future<http.Response> _achievedId(String route) async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    String url = pref.getString('url');
    String token = pref.getString('token');
    String uri =
        Uri.encodeFull(url + "/" + route + "/done/" + achievedId.toString());
    print(uri);
    var response = await http
        .patch(uri, headers: {'Accept': '*/*', 'x-access-token': token});
    return response;
  }
  ...
  Future<http.Response> _deleteId(String route) async {
    ...
    print(uri);
    var response = await http
        .delete(uri, headers: {'Accept': '*/*', 'x-access-token': token});
    return response;
  }
      ...
      secondaryActions: <Widget>[
            ...
            if (nextPage == 2) {
              ...
              Navigator.push(
                  ...
                  new MaterialPageRoute(
                      builder: (context) => MyReminderPage(
                            id: id.toString(),
                            dateText: date,
                            amountText: amount,
        ...
        IconSlideAction(
          ...
          onTap: () async {
            ...
            if (nextPage == 2) {
              Tab.index = 2;
              achievedId = id.toString();
              print(deleteId);
              var response = await _achievedId("reminder");
              print(response.body);
              if (response.statusCode == 200) {
                Fluttertoast.showToast(msg: "Success");
                Navigator.push(context,
...
class SildeAbleRowGoal extends StatelessWidget {
  ...
  Future<http.Response> _updateGoal() async {
    ...
    print(uri);
    print(id);
    var bodyEncode = json.encode({"id": id.toString(), "amount": amt.text});
    print(bodyEncode);
    ...
    return Slidable(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          trailing: Column(
            children: <Widget>[
              Padding(padding: EdgeInsets.fromLTRB(0, 14, 10, 0),
                  child: Text(date, style: GoogleFonts.lato(fontSize: 18, color: Colors.indigo, fontWeight: FontWeight.bold),)),
            ],
            ),
          title: Column(
            ...
            children: <Widget>[
              Padding(
                ...
                child: Text('₹ ' + amountAchieved.toString() + ' out of',
                  style: GoogleFonts.lato(
                    fontSize: 16,color: Colors.red
                  ),
                ),
              ),
          Text('₹ ' + amount.toString() + " saved ",style: GoogleFonts.lato(
            fontSize: 18),),
            ],
          ),
          subtitle: Padding(
      ...
      secondaryActions: <Widget>[
        ...
        IconSlideAction(
          ...
          onTap: () {
            print(
                date + amountAchieved.toString() + amount.toString() + purpose);
            Navigator.push(
                context,
                new MaterialPageRoute(
