...
def create_user():
    ...
    db.session.commit()
    token = jwt.encode({'public_id' : new_user.public_id, 'exp' : datetime.utcnow() + timedelta(minutes=300000)}, app.config['SECRET_KEY'])
    token = jwt.encode({'public_id' : new_user.public_id}, app.config['SECRET_KEY'])
