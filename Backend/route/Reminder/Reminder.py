...
def add_reminder(current_user):
  ...
  due_date = datetime.datetime.strptime(request.json['due_date'] , "%d-%m-%Y")
...
def get_reminders(current_user):
  ...
  today_3back = today - datetime.timedelta(days=3)
  user_rem = User.query.filter_by(public_id = current_user.public_id).first()
  result = user_rem.reminders.filter(func.DATE(Reminder.due_date)>=today_3back).order_by(Reminder.due_date.asc())
...
def update_reminder(current_user):
  ...
  try:
      reminder.due_date = datetime.datetime.strptime(request.json['due_date'],"%d-%m-%Y")
  except:
      reminder.due_date = datetime.datetime.strptime(request.json['due_date'],"%Y-%m-%d")
