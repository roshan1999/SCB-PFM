...
def get_reminders_notification(current_user):
  ...
  result = user_rem.reminders.filter(func.Date(Reminder.due_date)<=date3 , func.Date(Reminder.due_date) > today , Reminder.achieved == False).order_by(func.DATE(Reminder.due_date))
